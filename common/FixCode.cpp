...
void FixCoder::AddRoute(const string &compId, const string &customId) {
...
string FixCoder::GetTargetCompID(const FixMessage &msg, string &error) const {
  // get Tag 128/deliverTo id
  string deliverTo = FixHelper::GetTagValue(msg, FixHelper::TAG_DELIVER_TO_COMP_ID);
...
bool FixCoder::TransformMessage(FixMessage &msg, string &error) {
  // get labels
  string senderCompId = FixHelper::GetTagValue(msg, FixHelper::TAG_SENDER_COMP_ID);      // Tag 49
  string deliverToId = FixHelper::GetTagValue(msg, FixHelper::TAG_DELIVER_TO_COMP_ID);   // Tag 128
  ...
  string newTargetCompId;
  string newOnBehalfOfId;
